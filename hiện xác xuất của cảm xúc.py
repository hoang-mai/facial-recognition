...
def classify_image(image, args):
    ...
    with torch.no_grad():
        ...
        emotions_prob = {class_labels[i]: rounded_scores[i] for i in range(len(class_labels))}
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            for i, (emotion1, prob) in enumerate(emotion.items()):
                ...
                y_offset = i * 20  # Điều chỉnh vị trí văn bản
                img = cv2.putText(img, text, (x + 10 + w, y + y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
